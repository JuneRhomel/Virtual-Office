...
router.put('/', cookieJwtAuth, (req, res) => {
    ...
    try {
        const update = () => {
            return new Promise((resolve, reject) => {
                connection.query('UPDATE `role` SET ? WHERE id = ?', [data, id], (err, results) => {
                    if (err) {
                        reject(err);
                    } else {
                        resolve(results);
...
router.delete('/', cookieJwtAuth, (req, res) => {
    try {
        ...
        const dataDelete = () => {
            return new Promise((resolve, reject) => {
                connection.query('UPDATE `role` SET `deleted` = 1 WHERE `id` = ?', [id], (err, results) => {
                    if (err) {
                        reject(err);
                    } else {
                        resolve(results);
...
router.get('/:id', cookieJwtAuth, (req, res) => {
    try {
        let id = req.params.id; // Parse the id pararole from the URL
        id = decrypt(id)
        if (isNaN(id) || id <= 0) {
            return res.status(400).json({
                success: false,
                message: 'Invalid id pararole',
            });
        }
