...
router.get('/', cookieJwtAuth, (req, res) => {
    try {
        ...
        connection.query('SELECT * FROM `meter`', (err, results) => {
            if (err) {
                res.status(500).send(
                    {
                        success: false,
    ...
    try {
        const insert = () => {
            return new Promise((resolve, reject) => {
                connection.query('INSERT INTO meter set ?', data, (err, results) => {
                    if (err) {
                        reject(err);
                    } else {
                        resolve(results);
...
router.put('/', cookieJwtAuth, (req, res) => {
    ...
    try {
        const update = () => {
            return new Promise((resolve, reject) => {
                connection.query('UPDATE `meter` SET ? WHERE id = ?', [data, id], (err, results) => {
                    if (err) {
                        reject(err);
                    } else {
                        resolve(results);
...
router.delete('/', cookieJwtAuth, (req, res) => {
    try {
        ...
        const dataDelete = () => {
            return new Promise((resolve, reject) => {
                connection.query('UPDATE `meter` SET `deleted` = 1 WHERE `id` = ?', [id], (err, results) => {
                    if (err) {
                        reject(err);
                    } else {
                        resolve(results);
